...
import { ChefHat, Sparkles, AlertCircle, ArrowLeft } from 'lucide-react';
import { IngredientInput } from './components/IngredientInput';
import { PreferencesPanel } from './components/PreferencesPanel';
import { RecipeCard } from './components/RecipeCard';
import { RecipeModal } from './components/RecipeModal';
import { Ingredient, Recipe, UserPreferences, LoadingState } from './types';
...
const App: React.FC = () => {
  ...
  const handleReset = () => {
      setView('home');
      setRecipes([]);
      // We keep ingredients and preferences for better UX
  };
      ...
      <main className="max-w-5xl mx-auto px-4 mt-8">
        ...
        {error && (
            <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl mb-6 flex items-start gap-3 animate-slideIn">
                ...
                <p>{error}</p>
                <button onClick={() => setError(null)} className="ml-auto hover:text-red-900"><div className="sr-only">Dismiss</div>✕</button>
            </div>
        )}
